...
SharedHandle.prototype.add = function(worker, send) {
  ...
  console.log('ADDED', worker.id, 'at index', this.workers.indexOf(worker));
  send(this.errno, null, this.handle);
};

SharedHandle.prototype.remove = function(worker) {
  var index = this.workers.indexOf(worker);
  console.log('SEARCHING FOR', worker.id);
  if (index === -1) {
    console.error('The worker was not sharing this handle.');
    return false; // The worker wasn't sharing this handle.
  }
  console.log('FOUND AT', index);
  this.workers.splice(index, 1);
  if (this.workers.length !== 0) {
    console.error('There are this many other workers:', this.workers.length);
    return false;
  }
  this.handle.close();
  this.handle = null;
  console.error('Handle closed.');
  return true;
};
...
function masterInit() {
  ...
  function queryServer(worker, message) {
    ...
    console.error('ADDING', worker.id, key);
    handle.add(worker, function(errno, reply, handle) {
      ...
      if (errno) console.error('ERRNO', errno);
